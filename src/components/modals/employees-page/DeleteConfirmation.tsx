...
export default function DeleteConfirmation({ title, desc }: Props) {
  return (
      ...
      <div className="w-full h-full flex flex-col gap-y-1 justify-center items-center right-0 bg-[#000000]/70 backdrop-blur-md">
        <div className="bg-white py-6 px-5 rounded-lg w-[90%] space-y-3 lg:max-w-145 lg:p-8">
          ...
          <div className="flex items-center h-12.5 gap-x-2 mt-5 md:w-70 md:place-self-end">
            ...
            <button className="w-full capitalize h-full rounded-xl bg-[#DC2828] text-white focus:outline-none cursor-pointer transition ease-in-out delay-100 disabled:bg-gray-300 disabled:cursor-not-allowed md:w-1/2">
              delete
            </button>
          </div>
        </div>
